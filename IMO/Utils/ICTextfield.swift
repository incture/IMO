...
protocol UIDropDownDelegate: UITextFieldDelegate {
    @objc optional func datePickerDidSelect(row: Int, tag : Int)
}
...
final class ICTextField: UITextField {
    ...
    let datePicker: UIDatePicker = {
        ...
        dateDropDown.backgroundColor = UIColor.white
    ...
    @objc fileprivate func donePressed(_ sender : UIBarButtonItem) {
        if self.isDatePicker{
            ...
            if self.selectedDropDownItem == nil{
                ...
                self.dropDownDelegate?.datePickerDidSelect?(row: 0, tag: self.tag)
            }
            else{
                self.setSelectedItem(row: self.selectedDropDownItem.0)
                self.dropDownDelegate?.datePickerDidSelect?(row: self.selectedDropDownItem.0, tag: self.tag)
            }
    ...
    @objc private func cancelPressed(_ sender : UIBarButtonItem) {
        if self.isDropDown{
            self.selectedDropDownItem = nil
        }
        self.resignFirstResponder()
...
extension ICTextField: UIPickerViewDelegate {
    ...
    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        ...
        //self.dropDownDelegate?.datePickerDidSelect?(row: row, tag: self.tag)
    }
