...
public class CompressorDowntimeCaptureDao {

	private static final Logger logger = LoggerFactory.getLogger(CompressorDowntimeCaptureDao.class);

	public CompressorDowntimeCaptureDao() {
		super();
	}
...
//			logger.error("[insertDataInDB] : INFO - StartDate " + downtimeCaptureDo.getStartDate() + " EndDate " + downtimeCaptureDo.getEndDate());
	...
	public void deleteDataFromDB(Connection connection, int merrickId, String originalDateEntered) throws Exception {
		...
		try {
			...
			if (updatedRowCount > 0) {
				logger.error("[deleteDataFromDB] : INFO - Record Deleted succesfully");
			}

		} catch (Exception e) {
			logger.error("[deleteDataFromDB] : ERROR- Exception while updating data in database " + e);
			throw e;
		} finally {
			try {
				stmt.close();
			} catch (SQLException e) {
				logger.error("[deleteDataFromDB] : ERROR- Exception while cleaning environment" + e);
			}
		}
	}
