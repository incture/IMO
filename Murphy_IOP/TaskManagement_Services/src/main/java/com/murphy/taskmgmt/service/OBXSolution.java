...
public class OBXSolution {
	

    int NoOfVehicles;
    int NoOfCustomers;
    OBXVehicles[] Vehicles;
    double Cost;

    //Tabu Variables
    public OBXVehicles[] VehiclesForBestSolution;
    double BestSolutionCost;

    public ArrayList<Double> PastSolutions;

    OBXSolution(int CustNum, int VechNum , int VechCap)
    {
        this.NoOfVehicles = VechNum;
        this.NoOfCustomers = CustNum;
        this.Cost = 0;
        Vehicles = new OBXVehicles[NoOfVehicles];
        VehiclesForBestSolution =  new OBXVehicles[NoOfVehicles];
        PastSolutions = new ArrayList();

        for (int i = 0 ; i < NoOfVehicles; i++)
        {
            Vehicles[i] = new OBXVehicles(i+1,VechCap);
            VehiclesForBestSolution[i] = new OBXVehicles(i+1,VechCap);
        }
    }
        ...
        while (UnassignedCustomerExists(Nodes)) {
            ...
            for (int i = 1; i <= NoOfCustomers; i++) {
                if (Nodes[i].IsRouted == false) {
                	//timeDemand = 20 +CostMatrix[Vehicles[VehIndex].CurLoc][i];
    ...
    public void SolutionPrint(String Solution_Label)//Print Solution In console
    {
        ...
        for (int j=0 ; j < NoOfVehicles ; j++)
        {
            ...
            {   System.out.print("Vehicle " + j + ":");
                ...
                for (int k = 0; k < RoutSize ; k++) {
                    ...
                    { System.out.print(Vehicles[j].Route.get(k).NodeId ); 
                     // System.out.print(" -->code : "+Vehicles[j].Route.get(k).Code );
                    }
                    else
                    { System.out.print(Vehicles[j].Route.get(k).NodeId+ "->"); 
                     // System.out.print(" -->code : "+Vehicles[j].Route.get(k).Code );
            ...
            System.out.println("Load" +Vehicles[j].load);
        }
        System.out.println("\nSolution Cost "+this.Cost+"\n");
    }
    
    public List<String> SolutionData()//Print Solution In console
    {
        ...
        for (int j=0 ; j < NoOfVehicles ; j++)
        {
            ...
            {  
                ...
                for (int k = 0; k < RoutSize ; k++) {
                    if (k == RoutSize-1)
                    { 
                    	sequenceList.add(Vehicles[j].Route.get(k).Code);
                    //	System.out.print(Vehicles[j].Route.get(k).NodeId ); 
                     // System.out.print(" -->code : "+Vehicles[j].Route.get(k).Code );
                    }
                    else
