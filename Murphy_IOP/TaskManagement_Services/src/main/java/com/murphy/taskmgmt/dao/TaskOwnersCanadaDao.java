...
@Repository("TaskOwnersCanadaDao")
@javax.transaction.Transactional
public class TaskOwnersCanadaDao extends BaseDao<TaskOwnersDo, TaskOwnersDto> {
	...
	private static final Logger logger = LoggerFactory.getLogger(TaskOwnersCanadaDao.class);
	...
	public String createOwner(TaskOwnersDto dto) {
		...
		try {
			...
			logger.error("[Murphy][TaskOwnersCanadaDao][createOwner][error]" + e.getMessage());
		}
		return reponse;

	}

	public String updateOwners(List<TaskOwnersDto> owners, String taskId, String locationCode, String origin, String upliftFactor) {
		...
		try {
			if (!ServicesUtil.isEmpty(owners)) {
				for (TaskOwnersDto dto : owners) {
					...
					if (!ServicesUtil.isEmpty(existingOwners) && !existingOwners.contains(dto.getTaskOwner())) {
						dto = getOwnerDtoWithPriority(dto, locationCode, getResolveTimeByTaskId(dto.getTaskId()), upliftFactor);
						if (!createOwner(dto).equals(MurphyConstant.SUCCESS)) {
							respone = MurphyConstant.FAILURE;
						}
					} else if (!ServicesUtil.isEmpty(existingOwners) && existingOwners.contains(dto.getTaskOwner())) {
						if (!updateDriveTime(dto, upliftFactor).equals(MurphyConstant.SUCCESS)) {
							respone = MurphyConstant.FAILURE;
						}
			...
			logger.error("[Murphy][TaskOwnersCanadaDao][updateOwners][error]" + e.getMessage());
		}
		return respone;
	}
	
	private String updateDriveTime(TaskOwnersDto dto, String upliftFactor){
		if(!ServicesUtil.isEmpty(upliftFactor)){
			Double driveTime = dto.getEstDriveTime()+dto.getEstDriveTime()*(Double.parseDouble(upliftFactor)/100.0);
			Double totalTime = dto.getEstResolveTime() + driveTime;
			dto.setEstDriveTime(driveTime);
			dto.setEndTime(ServicesUtil.getDateWithInterval(dto.getStartTime(), totalTime.intValue(), MurphyConstant.MINUTES));
		}
		...
		try {
			...
			logger.error("[Murphy][TaskOwnersCanadaDao][updateDriveTime][error]" + e.getMessage());
		}
		return reponse;
	...
	private String deleteNonExistingTasks(List<String> instanceList, String taskId) {
		...
		try {
			...
			logger.error("[Murphy][TaskOwnersCanadaDao][deleteNonExistingTasks][error] - " + e.getMessage());
		}
		return response;
	}
	...
	public String updateStartEndTime(String userId, Date startTime, Date endTime, String taskId, double estDriveTime) {
		...
		try {
			...
			logger.error("[Murphy][TaskOwnersCanadaDao][updatePriority][query]  " + query);

			Query q = this.getSession().createSQLQuery(query);
			Integer result = (Integer) q.executeUpdate();
			response = MurphyConstant.SUCCESS;
		} catch (Exception e) {
			logger.error("[Murphy][TaskOwnersCanadaDao][updatePriority][error]  " + e.getMessage());
		}
		return response;
	...
	//
	// logger.error("[Murphy][TaskOwnersCanadaDao][updatePriorityWhenResolved][query]
	// " +
	// query);
	//
	// Query q = this.getSession().createSQLQuery(query);
	...
	// response = MurphyConstant.SUCCESS;
	// }catch(Exception e){
	// logger.error("[Murphy][TaskOwnersCanadaDao][updatePriorityWhenResolved][error]
	// " +
	// e.getMessage());
	// }
	// return response;
	...
	public String intialUpdateOfPriority() {
		...
		try {
			...
			logger.error("[Murphy][TaskOwnersCanadaDao][updatePriorityWhenResolved][error]  " + e.getMessage());
		}
		return response;
	}
	...
	public String updatePriority(String userId, int currentPosition, int newPosition, String taskId) {
		...
		try {
			...
			// logger.error("[Murphy][TaskOwnersCanadaDao][updatePriority][query New]
			// " + query);
	...
	public String updatePriorityWhenResolved(String taskId) {
		...
		try {
			...
			logger.error("[Murphy][TaskOwnersCanadaDao][updatePriorityWhenResolved][query]  " + query);
	...
	public TaskOwnersDto getOwnerDtoWithPriority(TaskOwnersDto ownerDto, String locationCode, float resolveTime, String upliftFactor) {
		...
		try{
			...
			if (driveTime != -1) {
				// SOC - updating drive time with uplift factor - Prakash Kumar
				if (!ServicesUtil.isEmpty(upliftFactor)) {
					driveTime = driveTime + (driveTime * Double.parseDouble(upliftFactor)/ 100.0);
					logger.error("[Murphy][TaskOwnersCanadaDao][getOwnerDtoWithPriority] Uplift Factor: "+ upliftFactor+" drive time with uplift factor: "+ driveTime);
				}
				// EOC - uplift factor
			...
			logger.error("[Murphy][TaskOwnersCanadaDao][getOwnerDtoWithPriority][error]  " + e);
		}
		return ownerDto;
	}
	...
	public String updateTaskDetailsOnShifting(TaskSchedulingUpdateDto dto) {
		...
		if (!ServicesUtil.isEmpty(dto.getShiftedTaskTaskId())
			...
			try {
				...
				logger.error("[Murphy][TaskOwnersCanadaDao][updatePriority][query New] " + query);

				Query q = this.getSession().createSQLQuery(query.toString());
				Integer result = (Integer) q.executeUpdate();
				
//				Updating start time for future tasks
				logger.error("[Murphy][TaskOwnersCanadaDao][updatePriority] 01: " + dto.getShiftedTaskStartDate());
				try {
					...
					logger.error("[Murphy][TaskOwnersCanadaDao][updatePriority][error] Not a future task " + e.toString());
				}
	...
	public StartTimeResponseDto getNewStartOfUser(String userId, String locationCode, float resolveTimeInSecs, Double driveTimeFromOBX) {
		...
		try {
			...
			logger.error("[Murphy][TaskOwnersCanadaDao][getNewStartOfUser][query] " + query);
			...
			logger.error("[Murphy][TaskOwnersCanadaDao][getNewStartOfUser][Exception] " + e);
			logger.error("[Murphy][TaskOwnersCanadaDao][getNewStartOfUser][Message Exception] " + e.getMessage());
		}
		logger.error("[Murphy][TaskOwnersCanadaDao][getNewStartOfUser][responseDto]" + responseDto);
		return responseDto;
	...
	public String getGainTimeOfUser(String userId) { // 1.if no task present for
		...
		try {
			...
			logger.error("[TaskOwnersCanadaDao][getGainTimeOfUser][Exception]" + e.getMessage());
		}
		return timeInString;
	}
	...
	public String updateOBXOwners(List<TaskOwnersDto> owners, String taskId, String locationCode, String origin) {
		...
		try {
			...
			tx.rollback();
			logger.error("[Murphy][TaskOwnersCanadaDao][updateOBXOwners][error]" + e.getMessage());
		} finally {
			try {
				if (session != null) {
					session.close();
	...
	public String updateStatus(String status, String taskId) {
		...
		try {
			...
			logger.error("[Murphy][TaskOwnersCanadaDao][updateStatus][error]  " + e.getMessage());
		}
		return response;
	...
	try {
		...
		logger.error("[Murphy][TaskOwnersCanadaDao][updateOwnersStartTime][QueryOwner]  " + QueryOwner);
		...
		logger.error("[Murphy][TaskOwnersCanadaDao][updateOwnersStartTime][endDateQuery]  " + endDateQuery);
		...
		logger.error("[Murphy][TaskOwnersCanadaDao][updateOwnersStartTime][UpdateTime]  " + UpdateTime);
		...
		logger.error("[Murphy][TaskOwnersCanadaDao][updateOwnersStartTime][error]  " + e.getMessage());
	...
	public void updateRevisionOfProcess(String taskID){
		...
		try {
			...
			logger.error("[Murphy][TaskOwnersCanadaDao][updateRevisionOfProcess][getRevisionQuery]  " + getRevisionQuery);
			...
			logger.error("[Murphy][TaskOwnersCanadaDao][updateRevisionOfProcess][queryUpdate]  " + queryUpdate);
			Query q = this.getSession().createSQLQuery(queryUpdate);
			int result = q.executeUpdate();
		} catch (Exception e) {
			logger.error("[Murphy][TaskOwnersCanadaDao][updateRevisionOfProcess][error]  " + e.getMessage());
		}
	} 
