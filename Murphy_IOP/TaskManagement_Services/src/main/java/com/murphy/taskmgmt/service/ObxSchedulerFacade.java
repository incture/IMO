...
public class ObxSchedulerFacade implements ObxSchedulerFacadeLocal {

	private static final Logger logger = LoggerFactory.getLogger(ObxSchedulerFacade.class);
		...
		try {
			...
			if (MurphyConstant.SUCCESS.equals(bestRoute.getResponseMessage().getStatus())) {
				for (int i = 1; i < bestRoute.getBestSequenceList().size(); i++) {
					...
					// Response from Arcgis in minutes and same stored in obx_task_allo
	...
	public void assignUsersToCluster(int dayOfWeek) {
		try {
			...
			for (String role : obxRoles) {
				...
				/*// SOC sort users based on competency
				logger.error("role: " + " " + roleList);
				//List<GroupsUserDto> users = userIdpMappingDao.getUsersBasedOnRole(roleList);
				List<GroupsUserDto> users = userIdpMappingDao.getUsersBasedOnRoleForOBX(roleList); //fetch along with competency
				Collections.sort(users, new OBXCompetencyComparator());
				for (GroupsUserDto userDto : users) {
					logger.error("user after sorting: " + " " + userDto.getUserId() + " " + userDto.getObxDesignation()
							+ " " + userDto.getFirstName() + " " + userDto.getLastName() + " " + userDto.getUserId()
							+ " " + userDto.getUserName() + " " + userDto.getpId());
				}
				// EOC sort users based on competency
*/				
				...
				for (Entry<Integer, List<ObxTaskDto>> entry : clusterTaskMap.entrySet()) {
					if (i < users.size()) {
						...
						logger.error("user after sorting before assigning " + " " +userID);
		...
		try {
			...
			for (Entry<String, List<ObxTaskDto>> userTask : userTasksMap.entrySet()) {
				...
				for (ObxTaskDto task : userTask.getValue()) {
					createOBXTask(userTask.getKey(), task.getLocationCode(),task.getDriveTime());
				}
			}
	...
	@Override
	public ResponseMessage createOBXTask(String userId, String location,Double driveTimeOBX) {
		...
		try {
			...
			//For Drive Time OBX so as not to call for ArcGis
			taskEventsDto.setDriveTime(driveTimeOBX);
	...
	public NearByTaskListDto getNearbyAssignedTask(double latitude, double longitude, String userId) {
		...
		if (latitude == 0.0 || longitude == 0.0) {
			...
			for (Iterator<NearbyTaskDto> iterator = list.iterator(); iterator.hasNext();) {
				...
				float estimatedCompletionTime = 0 ;
		...
		try {
			...
			if (currentTime.before(endOfDay)) {
				if (!ServicesUtil.isEmpty(dto.getTaskOwnerEmail()) && !ServicesUtil.isEmpty(dto.getUpdatedByEmail())
						&& !ServicesUtil.isEmpty(dto.getSelectedDay())
						&& !ServicesUtil.isEmpty(dto.getLocationCode())) {
					int day = getDayOfWeek(dto.getSelectedDay());
					// SOC for Mass Reassignments
					// List<String> exsitingOwnerList =
					// obxDao.getExsitingOwnerList(dto.getLocationCodeList(),
					// day);
					...
					// for(String locCode : dto.getLocationCodeList()){
					// response =
					// obxDao.updateTaskForOperator(dto.getTaskOwnerEmail(),
					// dto.getUpdatedByEmail(),
					// locCode, day, dto.getIsObxUser());
					// }
					updateUserSequence(day, dto.getTaskOwnerEmail());
					if (!ServicesUtil.isEmpty(exsitingOwner)) {
						updateUserSequence(day, exsitingOwner);
					}
					// if (!ServicesUtil.isEmpty(exsitingOwnerList)) {
					// for(String existingOwnerName : exsitingOwnerList)
					// updateUserSequence(day, existingOwnerName);
					// }
	...
	public void CreateTaskAllocationField() {
		...
		try {
			...
			taskAllocationFieldCreation(days, users, obxRoleAndFieldMap);
		...
		// INC0080580 Fix : EOC
		arcGISUtil.mailDriveTimeOutliers();
	}
	...
	@Override
	public void createClusters(int dayOfWeek, List<String> field, int numberOfClusters, String obxRole) { // boolean
																											// takeTierC
		try {
			// takeTierC
			List<ObxTaskDto> dayTasks = obxDao.getVisitMatrixWithFieldForDay(dayOfWeek, field);
			...
			// Changes made for Reallocation of Revoked task
	...
	public void taskAllocationFieldCreation(List<Integer> days, List<GroupsUserDto> users,
			HashMap<String, ArrayList<String>> obxRoleAndFieldMap) {
		// , boolean takeTierC
		for (int day : days) {
			for (Entry<String, ArrayList<String>> roleAndFieldList : obxRoleAndFieldMap.entrySet()) {
				//obx work division by competency
				users = userIdpMappingDao.getUsersBasedOnRole(Arrays.asList(roleAndFieldList.getKey()));
				
				//users = userIdpMappingDao.getUsersBasedOnRoleForOBX(Arrays.asList(roleAndFieldList.getKey()));
				try {
		...
		try {
			...
			if (!ServicesUtil.isEmpty(unresolvedLocCodes)) {
				...
				if (!ServicesUtil.isEmpty(restDays)) {
					// logger.error("RestDays " + restDays);
					...
					if (!ServicesUtil.isEmpty(restLocCodes)) {
						...
						if (!ServicesUtil.isEmpty(restDaysTierCWellsList)) {
							...
							logger.error("revokedTierCWellsList " + revokedTierCWellsList.size()
									+ " restDaysTierCWellsList" + restDaysTierCWellsList.size()
									+ " allDaysTierCLocCodes" + allDaysTierCLocCodes.size());
							...
							// , takeTierC
							taskAllocationFieldCreation(restDays, users, obxRoleAndRestFieldMap);
						}
					}
				}
			} else {
	...
	public List<ObxTaskDto> getAllOBXAllo() {
		List<ObxTaskDto> list = null;
		try{
			list=obxTaskAllocationDao.getAllOBXAllo();
			
		}catch(Exception e){
			logger.error("[OBXSchedulerFacade][getAllOBXAllo]Exception"+e);
		}
		return list;
	}
	...
	public void setDriveTime(List<ObxTaskDto> list) {
		try{
			obxTaskAllocationDao.setDriveTime(list);	
		}
		catch(Exception e){
			logger.error("[OBXSchedulerFacade][setDriveTime]Exception"+e);
		}
