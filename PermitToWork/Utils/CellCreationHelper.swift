...
// Task-Management
//
// Created by SAP Cloud Platform SDK for iOS Assistant application on 30/03/18
//
...
extension String {
    func convertEpocToDate() -> String {
        return Date(timeIntervalSince1970: Double(exactly: Double(self) ?? 0)!/1000).toDateFormat(.taskMgmtFormat)
    }
}
class RequestURL{
    static let shared = RequestURL()
    func urlRequest(for urlString: String, method: String = "GET", body:[String:Any]?) -> URLRequest{
        let urlStringVal = urlString.replacingOccurrences(of: " ", with: "")
       let url = URL(string: urlStringVal)
       var urlRequest = URLRequest(url: url!)
       urlRequest.httpMethod = method
       
       BLLinkingUrl.defaultHeaders.forEach { (key, value) in
