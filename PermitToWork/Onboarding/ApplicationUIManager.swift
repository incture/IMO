...
class ApplicationUIManager: ApplicationUIManaging {
    ...
    func showApplicationScreen(completionHandler: @escaping (Error?) -> Void) {
        ...
        if let savedViewController = self._savedApplicationRootViewController {
            ...
            if let notifTypee = UserDefaults.standard.value(forKey: "notifcation") {
                NotificationCenter.default.post(name: Notification.Name(rawValue: "backGroundNotification"), object: "backGroundNotification")
            }
            AppDelegate.shared.didRequestForPushNotificationOperation?()
        } else {
            DispatchQueue.main.async {
              
                
                let vc = UIStoryboard(name: "launcher", bundle: Bundle.main).instantiateViewController(withIdentifier: "LauncherController") as! LauncherController
